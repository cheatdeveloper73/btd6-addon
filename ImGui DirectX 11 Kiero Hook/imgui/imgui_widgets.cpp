...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImU32 check_col = IM_COL32(46, 172, 104, 255);
    ...
    ImVec2 label_pos = ImVec2((check_bb.Max.x-2) + 7.f, (check_bb.Min.y-2) + 4.f);
    if (g.LogEnabled)
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items)
{
    ...
    bool value_changed = Combo(std::string("##" + std::string(label)).c_str(), current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, height_in_items);
    return value_changed;
}
